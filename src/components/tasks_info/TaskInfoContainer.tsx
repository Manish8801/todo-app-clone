...
const TaskInfoContainer = () => {
  ...
  if (selectedTodo) {
    ...
    return (
      <div className="custom-scroll overflow-y-auto selected-task-info flex flex-col bg-black rounded-2xl min-w-94 w-[35vw] h-[85vh] mx-4 gap-5 py-4 px-6">
        <div className="flex-center-between p-1 font-semibold text-grey">
          <div className="group cursor-pointer ">
            <div className="flex-center-center  gap-1 text-xs">
              <div>{selectedTodo.listName}</div>
              <div>&gt;</div>
              <div>{selectedTodo.priority}</div>
            </div>
            <div className=" bottom-0 group-hover:block hidden bg-grey h-[1px]"></div>
          </div>
        ...
        <div className="text-white w-full px-2 py-4 text-2xl h-fit font-bold h-fit leading-6 text-wrap">
          {selectedTodo.description}
        </div>
        <div className="inline-flex gap-4">
          <button className="hover:-translate-y-0.5 duration-200 outline-none cursor-pointer px-3 py-2 text-white text-sm font-semibold items-center inline-flex gap-2 bg-light-black rounded-full">
